...
Differences Between Standard Clusters and Public Resource Pools
---------------------------------------------------------------

-  Standard Clusters provide dedicated computing clusters and network resources for users. The Standard Clusters of different users are physically isolated, while public resource pools are only logically isolated. Compared with public resource pools, Standard Clusters feature better performance in isolation and security.
...
-  Standard Clusters allow you to customize the runtime environment of physical nodes, for example, you can upgrade GPU drivers. This function is not supported by public resource pools.

Differences Between New and Old Standard Clusters
-------------------------------------------------

-  In the old version, the Standard Clusters dedicated for development/training are separated from those dedicated for service deployment. In addition, the pools of the two types offer different functions and their user experience varies. In the new version, the Standard Clusters of the two types are unified. You only need to configure one or multiple job types. Then, the dedicated resource pool automatically supports the configured job type.
-  New Standard Clusters inherit all functions of the old ones and have greatly improved user experience in key functions such as purchasing and resizing a resource pool. Use new Standard Clusters for smooth, transparent experience.
-  Additionally, the new Standard Clusters offer enhanced functions, for example, allowing you to upgrade GPU drivers, view details about job queuing, and use one network for multiple pools. More new functions of the new Standard Clusters are coming soon.
